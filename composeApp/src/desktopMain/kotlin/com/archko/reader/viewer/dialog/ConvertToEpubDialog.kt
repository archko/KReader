...
fun ConvertToEpubDialog(
    ...
    val filePickerLauncher = rememberFilePickerLauncher(
        type = FilePickerFileType.Custom(listOf("azw3", "mobi")),
        selectionMode = FilePickerSelectionMode.Multiple
    ) { files ->
        scope.launch {
            val filePaths = files.map { it.file.absolutePath }
    ...
    fun convert() {
        scope.launch {
            ...
            if (result > 0) {
                toaster.show(
                    message = getString(Res.string.convert_successfully)
                        .format(result, System.getProperty("user.home")),
                    type = ToastType.Success,
                )
            } else {
                toaster.show(
                    message = getString(Res.string.convert_error),
                    type = ToastType.Error,
                )
            }
        }
