 ...
 */
public object ParseTextMain {

    private val txtParser: TxtParser = TxtParser()

    public fun parseAsText(bytes: ByteArray): String {
        val content = String(bytes)
        return txtParser.parseAsText(content)
    }

    public fun parseAsList(bytes: ByteArray, pageIndex: Int): List<ReflowBean> {
        val content = String(bytes)
     ...
     * 解析为spanned,可以直接使用
     */
    public fun parseAsHtmlList(bytes: ByteArray, pageIndex: Int): List<ReflowBean> {
        val content = String(bytes)
        val list = txtParser.parseAsList(content, pageIndex)
        for (reflowBean in list) {
            if (ReflowBean.TYPE_STRING == reflowBean.type) {
                reflowBean.data = Html.fromHtml(reflowBean.data, Html.FROM_HTML_MODE_LEGACY).toString()
            }
        }
        return list
    }
    ...
    public fun parseAsTextList(bytes: ByteArray, pageIndex: Int): List<ReflowBean> {
        val content = String(bytes)
    ...
    public class TxtParser {
        ...
        public fun parseAsList(content: String, pageIndex: Int): List<ReflowBean> {
            //println("parse:==>" + content)
            val sb = StringBuilder()
            val list = ArrayList<String>()
            var aChar: Char
        ...
        public fun parseAsTextList(content: String, pageIndex: Int): List<ReflowBean> {
            //println("parse:==>" + content)
            val sb = StringBuilder()
            val list = ArrayList<String>()
            var aChar: Char
        ...
        private fun isLetterDigitOrChinese(str: String): Boolean {
    ...
    private val SINGLE_WORD_FIX_REGEX = Regex("( \\n)")
    ...
    private var minImgHeight = 32f
        set
        get

    public fun decodeBitmap(
        base64Source: String,
        systemScale: Float,
        screenHeight: Int,
        screenWidth: Int,
