...
public class ImagesDecoder(private val files: List<File>) : ImageDecoder {
            ...
            if (isFullImage) {
                ...
                if (regionDecoder != null) {
                    ...
                    if (scaledRegion.width() > 0 && scaledRegion.height() > 0) {
                        // 解码区域
                        val options = BitmapFactory.Options().apply {
                            inSampleSize = calculateInSampleSizeForRegion(scaledRegion, outWidth, outHeight)
                        }
     ...
     * 为区域解码计算采样大小
     */
    private fun calculateInSampleSizeForRegion(region: android.graphics.Rect, reqWidth: Int, reqHeight: Int): Int {
