...
public object IntentFile {
    private const val PRIMARY_VOLUME_NAME: String = "primary"

    @JvmStatic
    public fun processIntentAction(intent: Intent, context: Context): String? {
     ...
     */
    public fun getRealPathFromURI(context: Context?, uri: Uri): String? {
     ...
     */
    public fun getSafeDocumentId(uri: Uri?): String? {
        if (uri == null) {
            return null
        }
    ...
    public fun getFullPathFromTreeUri(treeUri: Uri?, context: Context): String? {
        if (treeUri == null) {
            return null
        }

        // 修复：优先判断是否为 Tree URI，非 Tree URI 直接走文件解析逻辑
        if (!DocumentsContract.isTreeUri(treeUri)) {
            return getRealPathFromURI(context, treeUri)
        }
    ...
    public fun getVolumePath(volumeId: String?, context: Context): String? {
        try {
            val mStorageManager =
    ...
    public fun getVolumeIdFromTreeUri(treeUri: Uri?): String? {
    ...
    public fun getDocumentPathFromTreeUri(treeUri: Uri?): String? {
        val docId = getSafeDocumentId(treeUri) ?: return null
        val split: Array<String?> =
            docId.split(":".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
     ...
     */
    @JvmStatic
    public fun deleteDir(dirPath: String?): Boolean {
        if (dirPath.isNullOrEmpty()) {
            return false
        }
        val dir = File(dirPath)
        return if (dir.isDirectory) {
            var success = true // 修复：初始化 success 为 true
            val files = dir.listFiles()
            if (files != null) {
                for (file in files) {
                    success = if (file.isDirectory) {
                        success && deleteDir(file.absolutePath)
                    } else {
                        success && file.delete()
                    }
                }
    ...
    public fun createTempFile(uri: Uri, context: Context): File {
    ...
    public fun getExtensionName(filename: String?): String? {
        ...
        return if (dot > -1 && dot < filename.length - 1) {
            ...
            null // 修复：无扩展名时返回 null，而非原字符串
        }
    }

    @Throws(IOException::class)
    public fun copy(inputStream: InputStream?, output: File?) {
    ...
    public fun isText(path: String): Boolean {
    ...
    public fun isImage(path: String): Boolean {
    ...
    public fun isTiffImage(path: String?): Boolean {
        return !TextUtils.isEmpty(path) &&
                (path!!.endsWith(".jfif", true)
                        || path.endsWith(".jfif-tbnl", true)
                        || path.endsWith(".tif", true)
    ...
    public fun isMuPdf(path: String): Boolean {
        return path.endsWith(".pdf", true)
    ...
    public fun isEpub(path: String): Boolean {
        return path.endsWith(".epub", true)
    }

    public fun isMobi(path: String): Boolean {
    ...
    public fun isDocx(path: String): Boolean {
        return path.endsWith(".docx", true)
    }

    public fun isDjvu(name: String): Boolean {
