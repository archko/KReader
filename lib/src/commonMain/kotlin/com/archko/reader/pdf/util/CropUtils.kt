...
public object CropUtils {
    ...
    private fun getLeftBound(bitmap: ImageBitmap, avgLum: Float): Float {
        ...
        for (x in 0 until w step LINE_SIZE) {
            val white = isRectWhite(
                bitmap,
                x,
                LINE_MARGIN,
                x + LINE_SIZE,
                bitmap.height - LINE_MARGIN,
                avgLum
            )
            if (white) {
                whiteCount++
    ...
    private fun getRightBound(bitmap: ImageBitmap, avgLum: Float): Float {
        ...
        return if (whiteCount > 0) min(
            bitmap.width,
            x + 2 * LINE_SIZE
        ).toFloat() / bitmap.width else 1f
    }
    ...
    private fun getBottomBound(bitmap: ImageBitmap, avgLum: Float): Float {
        ...
        return if (whiteCount > 0) min(
            bitmap.height,
            y + 2 * V_LINE_SIZE
        ).toFloat() / bitmap.height else 1f
    }

    private fun isRectWhite(
        bitmap: ImageBitmap,
        l: Int,
        t: Int,
        r: Int,
